...
function preload(){
  ...
  life1 = loadImage("mask.png");
  life2 = loadImage("sani1.png");
  life3 = loadImage("hand.png");
  life4 = loadImage("healthyFood.png");
  ...
  backSound=loadSound("baaghi.mp3")
}
...
function draw() {
  ...
  if(gameState === PLAY){
    ...
    for (var i = 0; i < prayerGroup.length; i++) 
    {
      if (boy.isTouching(prayerGroup.get(i)) )
        {
          score+=10; 
          endGame() 
        }
    ...
    prayerGroup.setLifetimeEach(-1);
     
     obstaclesGroup.setVelocityXEach(0);
     lifesGroup.setVelocityXEach(0); 
     prayerGroup.setVelocityXEach(0);
...
function spawnLife() {
  ...
  if (frameCount % 80 === 0) {
    ...
    life.y = Math.round(random(150, 300));
...
function prayer() {
  //write code here to spawn the life
  if (frameCount % 180 === 0) {
    var prayer = createSprite(width+20,height-300,40,10);
    prayer.y = Math.round(random(100, 250));
    prayer.addImage(prayerImg)
   
    prayer.scale = 0.3;
    prayer.velocityX = -(6 + score/100);
    
     //assign lifetime to the variable
     prayer.lifetime = 600;
    
    //adjust the depth
    prayer.depth = boy.depth;
    boy.depth = boy.depth + 1;
    prayerGroup.add(prayer);
...
function endGame()
{
        textSize(40);
        stroke("white");
        text("Hey You Defeated the Virus!!!", 100, 100);
        obstaclesGroup.destroyEach();
        lifesGroup.destroyEach();
        prayerGroup.destroyEach();
        ground.velocityX = 0;
